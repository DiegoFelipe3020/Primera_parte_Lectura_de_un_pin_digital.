/*
   Fundacion Kinal
   Centro educativo tecnico laboral kinal
   Quinto perito
   Quinto electronica
   Codigo Tecnico: EB5AV
   Curso: Taller de electronica digital y reparacion de computadoras I
   Proyecto: Como leer un pin digital
   Dev: Diego Alejandro Felipe Chis
   Fecha: 12 de abril
*/
//Directivas del procesador
#define push_button 2 //Push botton conectado al pin2
#define led 3

void setup(){
  pinMode(push_button, INPUT); //Pin2 como entrada
  pinMode(led, OUTPUT);
  Serial.begin(9600);
  Serial.println("Lectura de un pulsador");
}

void loop() {
  //variable local, que se encarga de almacenar el valor (abierto o cerrrado)
  //del pulsador conectado al pin2
  bool estado_boton = digitalRead(push_button);
  if (estado_boton == HIGH) {
    //si lo he presionado
    digitalWrite(led, LOW);//apago la led
  } else {
    //si no lo he presionado
    digitalWrite(led, HIGH); //mantengo encendido el led
  }
}